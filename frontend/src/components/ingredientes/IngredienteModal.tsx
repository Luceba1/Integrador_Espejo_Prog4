...
export default function IngredienteModal({
  ...
  useEffect(() => {
    ...
    setStockCantidad(initialData ? String(initialData.stock_cantidad ?? 0) : "0");
    setUnidadMedidaId(initialData?.unidad_medida_id ? String(initialData.unidad_medida_id) : "");
  }, [initialData, open]);
  ...
  return (
    <Modal open={open} title={initialData ? "Editar ingrediente" : "Nuevo ingrediente"} onClose={onClose}>
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-200">Stock del ingrediente</label>
            <input
              type="number"
              min="0"
              step="0.001"
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-200">Unidad del stock</label>
            <select
        ...
        <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm font-semibold text-slate-200">
          <input type="checkbox" checked={esAlergeno} onChange={(event) => setEsAlergeno(event.target.checked)} />
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button type="button" onClick={onClose} className="rounded-2xl border border-white/10 px-4 py-3 text-slate-200 hover:bg-white/5">
            Cancelar
          </button>
          <button type="submit" disabled={saving} className="rounded-2xl bg-blue-500 px-4 py-3 font-semibold text-white hover:bg-blue-400 disabled:opacity-60">
